...
class GraphLogic:   
    ...
    def node_in_graph(self, graph, entity:GraphModelBase) -> bool:
        ...
        ret = self.__sparql_connection.select(['1'], graph, [Triple(TripleItemIri(entity.iri), StandardPredicates._class, entity.graph_entity_type)])
    ...
    def graph_set_equality(self, graph, entity1:GraphModelBase, entity2:GraphModelBase, equality_type = None):
        ...
        if not if_entity1_known:
            raise Exception(f'{entity1.iri} not registered as {entity1.graph_entity_type}')
        if not if_entity2_known:
            raise Exception(f'{entity2.iri} not registered as {entity2.graph_entity_type}')

        if self.__graph_include_metadata and not self.node_in_graph(graph, if_entity1_known):
            self.__sparql_connection.insert(graph, [Triple(TripleItemIri(if_entity1_known.iri), StandardPredicates._class, if_entity1_known.graph_entity_type)])

        if self.__graph_include_metadata and not self.node_in_graph(graph, if_entity2_known):
            self.__sparql_connection.insert(graph, [Triple(TripleItemIri(if_entity2_known.iri), StandardPredicates._class, if_entity2_known.graph_entity_type)])
    ...
    def reldb_get_node(self, entity:GraphModelBase):
        if entity.id != 0:
            check_exists = self.__sql_connection.find_by_id(entity.table_name, entity.id)
        ...
        if not entity.iri is None:
            check_exists = self.__sql_connection.find_by_field(entity.table_name, 'iri', entity.iri)
            if len(check_exists) != 0:
                check_exists = check_exists[0]
    ...
    def graph_get_or_create_node(self, graph, entity:GraphModelBase, label = '') -> TripleItemIri:
        ...
        if not if_exist is None:
            if self.__graph_include_metadata and not self.node_in_graph(graph, if_exist):
                self.__sparql_connection.insert(graph, [Triple(TripleItemIri(if_exist.iri), StandardPredicates._class, if_exist.graph_entity_type)])
            return TripleItemIri(if_exist.iri)

        #before creation check if iri already registered as another type
        if not entity.iri is None:
            opposite = Entities(iri=entity.iri) if entity.graph_entity_type == Predicates.graph_entity_type else Predicates(iri=entity.iri)
            if_opposite = self.reldb_get_node(opposite)
            if not if_opposite is None:
                raise Exception(f"iri {entity.iri} already registered as {opposite.graph_entity_type} not possible to register as {entity.graph_entity_type}")

        id = self.__sql_connection.insert(entity.table_name, entity.get_insert_values())
        entity.id = id

        if entity.iri is None:
            entity.refresh_iri()
            self.__sql_connection.update(entity.table_name, entity.id, entity.get_update_values())

        if self.__graph_include_metadata:
            self.__sparql_connection.insert(graph, [Triple(TripleItemIri(entity.iri), StandardPredicates._class, entity.graph_entity_type)])
            self.__graph_cache_node(graph, entity.iri)
