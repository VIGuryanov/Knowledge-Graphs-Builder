...
class Migration:
    ...
    def up(self):
        engine = self.sql_connection.engine
        connection = engine.connect()
        metadata = db.MetaData()

        entities = db.Table(Entities.table_name, metadata,
        ...
        predicates = db.Table(Predicates.table_name, metadata,
            db.Column('Id', db.Integer, primary_key=True, nullable=False, index=True),
            db.Column('iri', db.VARCHAR(), unique=True, nullable=False, index=True))
    ...
    def down(self):
        ...
        db.Table(Entities.table_name, metadata, autoload_with=engine).drop(engine, checkfirst=True)
        db.Table(Predicates.table_name, metadata, autoload_with=engine).drop(engine, checkfirst=True)
